 ...
 * @version 1.0
 */
public class DbdemoWithStop {
	private static Logger logger = LoggerFactory.getLogger(DbdemoWithStop.class);
	public static void main(String args[]){
		DbdemoWithStop dbdemo = new DbdemoWithStop();
...
//		args[1].charAt(0) == args[2].charAt(0);
	...
	public void scheduleTimestampImportData(boolean dropIndice){
		...
		elasticsearchOutputConfig.setIndex("newdbdemo"); //必填项
...
//		dataStream.destroy();//释放资源
		Thread t = new Thread(new Runnable() {
			...
			public void run() {
				try {
					Thread.sleep(30*1000l);//休眠30秒，关闭作业
					dataStream.destroy();//释放资源
